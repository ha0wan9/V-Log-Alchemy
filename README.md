...
*Based on the recovered Hasselblad Phocus 4.0.1 X2D rendering path, described against Hasselblad's Natural Colour Solution (HNCS) reference, published in v1.1.*

*   **`Luts/Hasselblad/Hasselblad_Standard_Phocus_X2D_VLog.cube`**
    *   **Style**: Hasselblad Standard.
    *   **Features**: A natural, true-to-life rendering with smooth tonal transitions, restrained but rich saturation, and film-like contrast. The goal is the Phocus/HNCS baseline: reliable colour across mixed subjects without pushing one subject class at the expense of another.
*   **`Luts/Hasselblad/Hasselblad_Nature_Phocus_X2D_VLog.cube`**
    *   **Style**: Hasselblad Nature.
    *   **Features**: The same Standard colour foundation with the captured Phocus Nature RGB gradation table. It keeps the HNCS emphasis on smooth transitions and believable colour, while adding a slightly fuller tone response for outdoor colour and saturated scenes.
