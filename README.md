...
*Based on the stable Phocus 4.0.1 X2D film-curve/gradation path, described against Hasselblad's Natural Colour Solution (HNCS) reference, published in v1.1.*

*   **`Luts/Hasselblad/Hasselblad_Standard_Phocus_X2D_VLog.cube`**
    *   **Style**: Hasselblad Standard.
    *   **Features**: V-Log/V-Gamut mapped through Hasselblad RGB and the captured Phocus Standard film curve for natural, true-to-life tone, smooth transitions, restrained but rich saturation, and film-like contrast.
*   **`Luts/Hasselblad/Hasselblad_Nature_Phocus_X2D_VLog.cube`**
    *   **Style**: Hasselblad Nature.
    *   **Features**: The same Standard curve foundation with the captured Phocus Nature RGB gradation table. It keeps the HNCS emphasis on smooth transitions and believable colour, while adding a slightly fuller tone response for outdoor colour and saturated scenes.
...
*   **Not included in the stable LUTs**: Phocus RAW `ColorCorrect` / CbCr tables. They were captured and confirmed to change with white balance, so they remain an experimental research path rather than the default V-Log LUT path.
...
The Hasselblad Phocus LUTs use the stable portion of the recovered path:
...
The recovered Phocus `ColorCorrect` / CbCr stage is available behind the generator's experimental `--include-color-correct` option, but it is not used for the stable published LUTs because it changes with white balance.
