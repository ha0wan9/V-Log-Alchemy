...
*   **Daylight-baked**: the Phocus `ColorCorrect` / CbCr stage changes with white balance. The published LUTs use the daylight table, which also covers the cloudy/shade range; tungsten and warm color science is available from `Tools/generate_hasselblad_vlog.py --include-color-correct` but is not published as a separate LUT.
...
The Phocus `ColorCorrect` / CbCr stage changes with white balance; the published LUTs bake in the daylight table (which also covers cloudy/shade), while tungsten and warm captures remain available from the generator's `--include-color-correct` option.
